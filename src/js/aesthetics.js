...
var DROPS = [["Score"], ["Accuracy"], ["True Score"]];
...
function update_chart(chart_num, drop_num) {
    var drop = document.getElementById('chart' + chart_num + 'select-' + drop_num);
    var old = DROPS[chart_num][drop_num];
    var sel = drop.value;

    // Make the chosen item hidden on all the other drops for that graph
    // and make the old option visible on all other drops for that graph
    for(var i in DROPS[chart_num]) {
        var temp_drop = document.getElementById('chart' + chart_num + 'select-' + i);
        if(i == drop_num) {
            for(var j = 0; j < temp_drop.length; j++) {
                if(temp_drop[j].value == sel)
                    temp_drop[j].hidden = true;
                else if(temp_drop[j].value == old)
                    temp_drop[j].hidden = false;
            }
        }
    }

    DROPS[chart_num][drop_num] = document.getElementById('chart' + chart_num + 'select-' + drop_num).value;
    if(chart_num == 0)
        redraw(CHART1, drop_num);
    else if(chart_num == 1)
        redraw(CHART2, drop_num);
    else
        redraw(CHART3, drop_num);
}

function redraw(chart, drop_num) {
...
function reload_data() {
    ...
    for(var j in list) {
        ...
        for(var i in DROPS[0]) {
            CHART1.data.datasets[i].data.push(data[DROPS[0]]);
        }
        for(i in DROPS[1]) {
            CHART2.data.datasets[i].data.push(data[DROPS[1]]);
        }
        for(i in DROPS[2]) {
            CHART3.data.datasets[i].data.push(data[DROPS[2]]);
        }
...
function create_chart() {
    ...
    for(var i in opts) {
        for(var j = 1; j <= 3; j++) {
            if(document.getElementById('chart' + j + 'select-0').value == opts[j])
                continue;
            ...
            document.getElementById('chart' + j + 'select-0').appendChild(opt);
        }
    }
